...
Temperatura <- read_csv("C:/usumacinta/Temperatura.csv")
...
View(Temperatura)
print(Temperatura)
#Leer Primeros registros de las tablas
head(Temperatura)
...
Temperatura$Fecha <- as.Date(Temperatura$Fecha, format = "%d/%m/%y")
#Convertir la columan a tipo númerico
Temperatura$Temperatura <- as.numeric(Temperatura$Temperatura_deg_c)
...
#------------------------------------------------------------#
# Extraer la variable que se desea obtener la descomposición #
#------------------------------------------------------------#
Temperatura_var <- Temperatura$Temperatura_deg_c
...
serie_temporal_diaria <- ts(Temperatura_var, start=c(1980, 1), end=c(2022, 365), freq=365)
...
serie_temporal_mensual <- ts(Temperatura_var, start=c(1980, 1), end=c(2022, 365), freq=12)
# Descompone las series temporales
...
Periodo_final <- subset(Temperatura, Fecha > as.Date("2020-12-31"))
# Visualizar el resumen de los datos de los últimos dos años
summary(Periodo_final)
Temperatura_2021_2022 <- as.numeric(Periodo_final$Temperatura_deg_c)
ts_diara_2021 <- ts(Temperatura_2021_2022, start=c(2021, 1), end=c(2022, 365), freq=365)
